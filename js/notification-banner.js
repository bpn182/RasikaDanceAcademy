...
class NotificationBanner {
  constructor() {
    this.bannerConfig = {
      ...
      message:
        "Applications for 2025 Arangetram ceremony are now open. Limited seats available for qualified students.",
  ...
  getBannerClasses() {
    const baseClasses =
      "fixed top-0 left-0 right-0 z-[9999] transform -translate-y-full transition-transform duration-500 ease-in-out shadow-lg";
    ...
    return `${baseClasses} ${
      typeClasses[this.bannerConfig.type] || typeClasses.event
    }`;
  }
  ...
  getBannerHTML() {
    ...
    const actionButton = this.bannerConfig.showButton
      ? `<a href="${this.bannerConfig.buttonLink}" ${
          this.bannerConfig.openInNewTab
            ? 'target="_blank" rel="noopener noreferrer"'
            : ""
        } class="banner-cta ml-4 bg-white/20 hover:bg-white/30 text-white px-4 py-1 rounded-full text-sm font-medium transition-colors">${
          this.bannerConfig.buttonText
        }</a>`
      : "";
  ...
  addEventListeners() {
    ...
    // Make entire banner clickable except dismiss button
    const bannerContent = this.bannerElement.querySelector(
    ...
    if (bannerContent) {
      bannerContent.addEventListener("click", (e) => {
        ...
        if (
          e.target.closest(".banner-dismiss") ||
          e.target.closest(".banner-cta")
        )
          return;
...
const bannerStyles = `
    <style>
        ...
        /* Push site nav down when banner is visible */
